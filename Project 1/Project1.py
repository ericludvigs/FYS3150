...
try:
    ...
    raise ValueError(
        ...
        "file to run = sys.argv[1]",
        "exponent = sys.argv[2]"
        )

# Run code, here c++ code  which has been compiled and linked
programpath = "CodeExamples/"
outputpath = "Output/"
subprocess.run(["./"+filename+".x", filename+"_out", str(exponent)], cwd=programpath)
# Move the output files
for i in range(1, exponent+1):
    os.rename(programpath+filename+"_out"+str(i), outputpath+filename+"_out"+str(i))

# Start making figures looping over all exponents
filename = "Output/"+filename+"_out"
for i in range(1, exponent+1):
